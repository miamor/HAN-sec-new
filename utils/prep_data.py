...
class PrepareData(object):
    ...
    def __init__(self, config):
        ...
        self.save_json = True
        if 'do_draw' in config:
            self.do_draw = config['do_draw']
        else:
            self.do_draw = False
        
        if 'graph_viz_dir' in config:
            self.graph_viz_dir = config['graph_viz_dir']
        else:
            self.graph_viz_dir = 'data/graphviz'
    ...
    def load_data_files(self, task_ids, report_file_name='report.json', write_to_json=True):
        ...
        Return self.data_dortmund_format
        """
        print('\nProcess data from one report file to json format and encode nodes & edges')
    ...
    def encode_node(self, node):
        ...
        if node['graph'] not in self.graphs_dict.keys():
            print('Append', node['graph'], 'to self.graphs_dict')
    ...
    def encode_edge(self, path):
        ...
        if len(path) <= 0:
            print("del self.graphs_dict[path['graph']]", path['graph'])
        ...
        if 'nodes' in self.json_data.keys():
            ...
            for node_id in self.json_data['nodes']:
                ...
                if node['graph'] != g_name:
                    ...
                    # print('\t node graph', node['graph'], g_name)
                    self.encode_node(node)
                    n_num += 1
                    if n_num % 100000 == 0 or n_num == n_tot:
                        print('{}/{}'.format(n_num, n_tot))
        ...
        for g_name in list(self.graphs_name_to_label.keys()):
            ...
            if not graph.edata:
                ...
                if self.do_draw:
                    ...
                    print('Visualize graph to {}/{}/{}'.format(self.graph_viz_dir, os.path.basename(self.reports_parent_dir_path), g_name))
